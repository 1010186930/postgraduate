...
{  
     static float Velocity,Encoder_Least,Encoder,Movement;
	  static float Encoder_Integral,Target_Velocity=90;
	  //=============遥控前进后退部分=======================// 
...
int turn(int encoder_left,int encoder_right,float gyro)//转向控制
{
    ...
    if(Turn_Target>Turn_Amplitude)  Turn_Target=Turn_Amplitude;    //===转向速度限幅
	  if(Turn_Target<-Turn_Amplitude) Turn_Target=-Turn_Amplitude;   //===转向速度限幅
		if(Flag_Qian==1||Flag_Hou==1)  Kd=0.4;                         //===接收转向遥控数据直立行走的时候增加陀螺仪就纠正    
		else Kd=0.27;                                          
  	//=============转向PD控制器=======================//
...
{ 		   
	...
	if(flag==0)                                                                   //第一步
	 ...
	 if(flag==2)                                                                  //第三步
	 {
	    ...
	    if(myabs(encoder_left+encoder_right)>150)                                 //条件3，小车的轮胎因为正反馈达到最大的转速   
      {
				flag=0;                                                                                     
				return 1;                                                               //检测到小车被拿起
			}
