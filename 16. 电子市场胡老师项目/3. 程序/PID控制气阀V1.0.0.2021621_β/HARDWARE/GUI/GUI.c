...
void Show_Adc_Average(u16 x, u16 y, u32 adcnum, double *voltage, u8 size)
{
	double temp;
	u16 adc_value;
	double show_num;
	...
	const char adc_unit[]="V";
	POINT_COLOR=BLACK;
//	if((set_voltagevalue-0)<=EPSILON)
//	{
//		adc_value=0;
//		LCD_Fill(x,y,x+60,y+20,WHITE);
//		LCD_ShowNum(x,y,adc_value,4,size);//显示ADC的值
//		POINT_COLOR=BLUE;
//		LCD_Fill(x,y+20,x+80,y+40,WHITE);
//		Show_Str(x,y+20,POINT_COLOR,BACK_COLOR,"0V",16,1);
//		LCD_Fill(5,y+60,x+80,y+80,WHITE);
//		POINT_COLOR=RED;
//		Show_Str(5,y+60,POINT_COLOR,BACK_COLOR,"0ml",16,1);//显示流量值
//	}
//	else
//	{
		adc_value=adcnum;
		LCD_Fill(x,y,x+60,y+20,WHITE);
		LCD_ShowNum(x,y,adc_value,4,size);//显示ADC的值
		temp=(double)(adc_value)*(3.29/2048);//真实电压值是(show_num)*(3.3/2048)的两倍
		*voltage=temp;
		sprintf(display,"%.3f",temp);
		strcat(display,adc_unit);
		LCD_Fill(x,y+20,x+80,y+40,WHITE);
		POINT_COLOR=BLUE;
		Show_Str(x,y+20,POINT_COLOR,BACK_COLOR,(u8*)display,16,1);
		show_num=VoltagetoFlow(*voltage);
		if(fabs(show_num-7000)>EPSILON&&show_num>7000)
			show_num=7000.000;
		LCD_Fill(5,y+60,x+80,y+80,WHITE);
		sprintf(display,"%.3f",show_num);
		strcat(display,valve_unit);
		POINT_COLOR=RED;
		if(fabs(show_num-0)>EPSILON&&show_num<0)
			Show_Str(5,y+60,POINT_COLOR,BACK_COLOR,"错误",16,1);//显示流量值
		else
			Show_Str(5,y+60,POINT_COLOR,BACK_COLOR,(u8*)display,16,1);//显示流量值
//	}
	adc_value=adcnum;
	temp=(double)(adc_value)*(3.29/2048);//真实电压值是(show_num)*(3.3/2048)的两倍
	*voltage=temp;
