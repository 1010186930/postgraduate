...
//电压转换为流量，二分法查找，重复数值会找到最后一个
double VoltagetoFlow(double voltage)
{
	return BinarySearch(voltage ,1);
}
//流量转换为电压，二分法查找，重复数值会找到最后一个
double FlowtoVoltage(double flow)
{
	return BinarySearch(flow ,2);
}
//PID调整采样电压，输出下一个时刻预期的采样电压
double PID_realize(double set_voltage, double voltage_adc)
{
    int index;
    pid.SetVoltage = -0.026;						//固定电压值传入
	pid.ActualVoltage = voltage_adc;				//实际电压传入 = ADC_Value * 3.3f/ 4096
    pid.err = pid.SetVoltage + 1.233 - pid.ActualVoltage;	//计算偏差
//调试使用：	
	sprintf(display_buf,"SetVoltage:%.3f",pid.SetVoltage);
			strcat(display_buf,"v\r\n");
			RS485_SendData((u8*)display_buf,strlen(display_buf));
	sprintf(display_buf,"ActualVoltage:%.3f",pid.ActualVoltage);
			strcat(display_buf,"v\r\n");
			RS485_SendData((u8*)display_buf,strlen(display_buf));
...
//调试使用：	
		sprintf(display_buf,"index:%d",index);
			strcat(display_buf,"v\r\n");
			RS485_SendData((u8*)display_buf,strlen(display_buf));
		sprintf(display_buf,"integral:%.3f",pid.integral);
			strcat(display_buf,"v\r\n");
			RS485_SendData((u8*)display_buf,strlen(display_buf));
    pid.voltage = pid.SetVoltage +1.233 +pid.Kp * pid.err + index * pid.Ki * pid.integral + pid.Kd * ( pid.err - pid.err_last);//位置式公式
//调试使用：	
